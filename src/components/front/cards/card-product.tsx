...
export default function CardProduct({ title, image, slug, category, colors, price, isSwiper }: { title: string, image: string, slug: string, category: string, colors: string[], price: string, isSwiper?: boolean }) {
    return (
                    ...
                    <div className="flex flex-wrap md:flex-nowrap items-center p-3 md:p-3 xl:p-4 bg-white rounded-[16px] md:rounded-[20px] lg:rounded-[22px] xl:rounded-[25px] overflow-hidden">
                        ...
                        <div className="w-full md:w-auto flex-shrink-0 flex items-center gap-x-2 mt-1 md:mt-0">
                            {colors.map((color, key) => (
                                <span key={key} className={`h-3 lg:h-5 w-3 lg:w-5 inline-block rounded-full ${color}`}></span>
                            ))}
                        </div>
                    </div>
                </div>
